...
export default function CanvasPriceChart({ data, width = "100%", height = 250 }) {
  ...
  useEffect(() => {
    ...
    const getX = (index) => PADDING.left + (index / (data.length - 1)) * CHART_WIDTH;
    const getY = (price) => PADDING.top + CHART_HEIGHT - ((price - yMin) / yRange) * CHART_HEIGHT;
    ...
    if (mousePos) {
        ...
        if (x >= PADDING.left && x <= w - PADDING.right) {
            ...
            ctx.textAlign = 'left';
            ctx.fillText(`${point.mid_price.toFixed(2)}`, tooltipX + 8, tooltipY + 16);
  ...
  return (
    <div ref={containerRef} className="panel chart-container" style={{ position: 'relative', padding: 0, overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 5, left: 10, zIndex: 10, pointerEvents: 'none' }}>
        <h4 style={{ margin: 0, fontSize: '12px', color: '#e2e8f0' }}>Price Action</h4>
      </div>
      <canvas 
        ref={canvasRef} 
